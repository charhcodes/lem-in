// bfs to find shortest path [not working]
...
// queue := []*Vertex{rooms[startRoom]}
// visited := map[string]bool{startRoom: true}
// distance := map[string]int{startRoom: 0}
...
// 				visited[neighbour.name] = true
// 				queue = append(queue, neighbour)
// 				distance[neighbour.name] = distance[vertex.name] + 1
// 			}
// 		}
// 	}
...
// // If we reach here, we couldn't find a path from startRoom to endRoom
// return -1

func bfs(start, end string, graph *Graph) int {
	// initialize the queue with the starting vertex
	queue := []*Vertex{graph.getVertex(start)}
	// mark the starting vertex as visited
	queue[0].visited = true
	// initialize the distances map with the starting vertex
	...
	for len(queue) > 0 {
		...
		// if we have reached the end vertex, return its distance from the start
		if currentVertex.name == end {
			return distances[end]
		}

		// iterate over the current vertex's neighbors
		for _, neighbor := range currentVertex.links {
			...
			if !neighbor.visited {
				...
				// set the neighbor's distance from the start
				distances[neighbor.name] = distances[currentVertex.name] + 1
			}
		}
	}

	// if we reach here, it means we were not able to reach the end vertex from the start
	// so we return -1 to indicate that it is not possible
	return -1
}
...
func main() {
	...
	distance := bfs(startroom, endroom, &test)
	fmt.Printf("Minimum distance required to get from %s to %s: %d\n", startroom, endroom, distance)
}
